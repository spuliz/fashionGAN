...
class ClothGAN2Model(Stage2BaseModel):
    ...
    def forward_AtoBencodeC(self):
        ...
        self.real_B_encoded_cloths, self.fake_B_encoded_cloths = self.generate_random_block(self.real_B_encoded,self.fake_B_encoded)
    ...
    def backward_EG(self):
        ...
        self.loss_G_L1 = self.criterionL1(self.fake_data_encoded[0], self.real_data_encoded[0]) * self.opt.lambda_p_l

        # 4. style loss
        self.loss_s_l = self.criterionS_l(self.fake_data_encoded[0],self.real_data_encoded[0]) * self.opt.lambda_s_l

        # 5. gram loss
        self.loss_g_l = self.criterionGLCM(self.fake_data_encoded[0],self.real_data_encoded[0])* self.opt.lambda_p_l
        ...
        self.loss_G = self.loss_G_GAN + self.loss_G_GAN2 + self.loss_kl + self.loss_G_L1 + self.loss_c
